...
#define CWS_HASH_TABLE_EXPONENT 14
#define CWS_HASH_TABLE_SIZE (size_t)(1<<CWS_HASH_TABLE_EXPONENT)
#define CWS_HASH_TABLE_SIZE_MSK (size_t)(CWS_HASH_TABLE_SIZE-1)
...
/*
size_t
cws_hash(const char *s)
{
  size_t h = 0;
  while (*s)
    h = *s++ + (h << 6) + (h << 16) - h; // Red Dragon book h = 65599*h + c
  return h&CWS_HASH_TABLE_SIZE_MSK;
}
